#' Fit delta-length (season-to-season) growth translation model
#'
...
#'
#' @export
fit_season_growth_model <- function(fish_al,
                                    target_season,
                                    maxage = 10L,
  ...
  req <- c("YEAR","REGION_GRP","SEASON","AGE_YRS","LENGTH")
  ...
  d <- d[is.finite(YEAR) & is.finite(SEASON) & is.finite(REGION_GRP) & is.finite(AGE_YRS) & is.finite(LENGTH)]
  #d <- d[QUARTER %in% 1:4]
  ...
  # grouping keys
  grp <- c("YEAR","AGE_YRS","REGION_GRP","SEASON")
  if (gear_mode == "by_gear") grp <- c(grp, "GEAR2")
  if (sex_mode  == "split")  grp <- c(grp, "SEX")

  # cell means by year/AGE_YRS/region/season/(gear/sex)
  cell <- d[, .(
    N_Q = .N,
    MEANL_RAW = mean(LENGTH, na.rm = TRUE)
  ), by = grp]

  # broader mean for shrinkage: (YEAR, AGE_YRS, REGION_GRP, SEASON) pooled over gear/sex if present
  broad_keys <- c("YEAR","AGE_YRS","REGION_GRP","SEASON")
  broad <- d[, .(
    N_BROAD = .N,
  ...
  # shrunk mean length in season
  cell[, MEANL_Q := W_CELL * MEANL_RAW + (1 - W_CELL) * MEANL_BROAD]

  # build MEANL at target season, same keys except season
  tgt <- cell[SEASON == target_season]
  if (nrow(tgt) == 0) {
    stop("No data available in target_season =", target_season, " to define mean length-at-age.", call. = FALSE)
  }

  # keys to match quarter-specific rows to target season rows
  match_keys <- setdiff(grp, "SEASON")
  ...
  if (any(is.na(deltas$MEANL_QSTAR))) {
    ...
    if (gear_mode == "by_gear") {
      ...
      tgt_drop <- cell[SEASON == target_season, .(MEANL_QSTAR = mean(MEANL_Q, na.rm = TRUE)), by = drop_keys]
    ...
    # 3) final fallback: year+region+age only
    tgt_drop <- cell[SEASON == target_season, .(MEANL_QSTAR = mean(MEANL_Q, na.rm = TRUE)), by = .(YEAR, REGION_GRP,AGE_YRS)]
    deltas <- merge(deltas, tgt_drop, by = c("YEAR","REGION_GRP","AGE_YRS"), all.x = TRUE, suffixes = c("", ".fb3"))
