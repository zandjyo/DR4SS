...
#' @param min_strata_n minimum N within each (YEAR,QUARTER,AGE,...) required; else strata dropped later if you want
#'
...
build_empirical_length_at_age <- function(fish_data,
  ...
  need <- c("YEAR","QUARTER","AGE","LENGTH")
  miss <- setdiff(need, names(dt))
  if (length(miss) > 0) stop("fish_data missing: ", paste(miss, collapse = ", "), call. = FALSE)

  al <- dt[!is.na(AGE) & !is.na(LENGTH) & !is.na(YEAR) & !is.na(QUARTER)]
