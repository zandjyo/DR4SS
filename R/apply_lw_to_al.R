...
#'
#' @return data.table with YEAR, QUARTER, AGE_YRS, (GEAR2), (SEX), WAA, N_AL
#' @export
apply_lw_to_al <- function(al_emp,
  ...
  if (!requireNamespace("data.table", quietly = TRUE)) stop("Need data.table.", call. = FALSE)
   ...
   al_emp$QUARTER<-as.factor(al_emp$QUARTER)
...
pred_keys <- c("YEAR", "QUARTER", "BIN_ID")
...
AL2 <- merge(
  ...
  by_cols <- c("YEAR","QUARTER","AGE_YRS")
  ...
  data.table::setorder(out, YEAR, QUARTER, AGE_YRS)
  out
}
