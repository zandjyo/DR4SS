...
#' @param by_sex Logical indicating whether to split the output by sex.
#'
...
#'
#' @export
survey_cond_age_length <- function(con_akfin,
                                species,
                                area,
                                start_year,
                                max_age,
  ...
  Age_sql <- sql_reader("survey_age_AKFIN.sql")
  ...
  # Recode SEX: 2 (female) -> 1, 1 (male) -> 2, others/NA -> 3
  if ("SEX" %in% names(Age)) {
    Age[, SEX := data.table::fifelse(SEX == 2L, 1L,
                              data.table::fifelse(SEX == 1L, 2L, 3L))]
  } else {
    Age[, SEX := 3L]
  }
  ...
  if (length(miss) > 0) {
    stop("survey_age_AKFIN.sql result missing required columns: ", paste(miss, collapse = ", "), call. = FALSE)
  }
  ...
  # Special case: Remove age 17 fish for cod in the Bering Sea
  if (species == 202 && area == "BS") {
    dt <- dt[AGE != 17]
  }

  # --- Filter by sex if requested ---
  if (by_sex) {
    # Split unknown sex (3) evenly between male (2) and female (1)
    dt_known <- dt[SEX %in% c(1L, 2L)]
    dt_unknown <- dt[SEX == 3L]
    if (nrow(dt_unknown) > 0) {
      dt_female <- copy(dt_unknown)[, SEX := 1L]
      dt_male   <- copy(dt_unknown)[, SEX := 2L]
      # If there is a weight/count column, halve it here. Otherwise, duplicate rows.
      dt <- rbindlist(list(dt_known, dt_female, dt_male))
      # If you have a count/weight column, e.g. N, do: dt_female[, N := N/2]; dt_male[, N := N/2]
    } else {
      dt <- dt_known
    }
  ...
  out_list <- list()
  for (sx in sexes) {
    dts <- dt[SEX == sx | (!by_sex & SEX == 3L)]

    if (nrow(dts) == 0) next

    # ---- plus group ----
    dts[, AGE_FILT := AGE]
    dts[AGE_FILT > max_age, AGE_FILT := max_age]
    dts[, AGE_FILT := as.integer(AGE_FILT)]

    # ---- counts by YEAR x BIN x AGE ----
    counts <- dts[, .N, by = .(YEAR, BIN, AGE_FILT)]
    ...
    Agecomp_obs[, 2] <- seas
    Agecomp_obs[, 3] <- flt
    Agecomp_obs[, 4] <- if (by_sex) sx else 3L   # <-- Set Sex column
    Agecomp_obs[, 5] <- 0
    ...
    out_list[[length(out_list) + 1]] <- Agecomp_obs
  }

  # Combine all sex splits (or just one if pooled)
  result <- do.call(rbind, out_list)

  # Set column names to match fishery_cond_age_length.R
  age_cols <- paste0("a", 0:max_age)
  colnames(result) <- c(
    "Yr", "Seas", "Flt", "Gender", "Part", "Ageerr",
    "Lbin_lo", "Lbin_hi", "Nsamp", age_cols
  )
