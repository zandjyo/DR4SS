#' Translate AL bin lengths to a target quarter using delta-length model
#'
...
#'
#' @export
translate_lengths_to_quarter <- function(al_emp,
                                        growth_fit,
                                        clamp = TRUE,
                                        min_L = NULL,
                                        max_L = NULL) {
  ...
  if (is.null(growth_fit$deltas)) stop("growth_fit must come from fit_quarter_growth_model().", call. = FALSE)

  AL <- DT(al_emp)
  names(AL) <- toupper(names(AL))

  req <- c("YEAR","AGE_YRS","QUARTER","BIN_ID","L_BAR","P_LEN","N")
  ...
  suppressWarnings({
    ...
    AL[, QUARTER := as.integer(as.character(QUARTER))]
