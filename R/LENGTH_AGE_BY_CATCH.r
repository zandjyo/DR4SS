...
#' @param species_srv RACE species code.
...
#' @param start_year first year to include, do not include any data prior to 1990.
...
#' @param age_length = c("LENGTH","AGE") whether to prepare length or age composition 
#' @param map_sample = c("MAP","sample") when returning integer ages, use "MAP" or "sample".
#' @param max_age Plus-group maximum age.
#'
#' @return
...
LENGTH_AGE_BY_CATCH<- function(con_akfin,
  ...
  if (start_year <= 1989) stop("`start_year` must be greater than 1989.", call. = FALSE)
  ...
  lfreq <- sql_filter(sql_precode ="<=", x = ly, sql_code = lfreq, flag = "-- insert end", value_type = c("numeric"))
  lfreq <- sql_filter(sql_precode =">=", x = start_year, sql_code = lfreq, flag = "-- insert start", value_type = c("numeric"))
  ...
  OBS_DATA<-OBS_DATA[YEAR <= ly & YEAR >= start_year]
  ...
  if (isTRUE(PORT)) {
    ...
    PORTAL <-PORTAL[YEAR <= ly & YEAR >= start_year]
   }

    # ---- B: 1999–2007 ----
    if(start_year<= 2007 & ly >= 1999 ){
    ...
    PORTBL<-PORTBL[YEAR>=start_year&YEAR<=ly]
    }
    ...
    PORTCL<-PORTCL[YEAR>=start_year&YEAR<=ly]
   }

    # ---- D: 2008–2010 ----
    if(start_year<= 2010 & ly >= 2008){
    ...
    PORTDL<-PORTDL[YEAR>=start_year&YEAR<=ly]
   }

  objs <- c("PORTAL", "PORTBL", "PORTCL", "PORTDL")

  existing <- objs[vapply(objs, exists, logical(1), inherits = TRUE)]

  PORT_DATA <-  if (length(existing) > 0) {
      do.call(rbind, lapply(existing, get))
  } else {
      stop("`There are no port data available for this date range/area and species, rerun without port data", call. = FALSE)
  }
   
  objs <- c("PORT_DATA", "OBS_DATA")
  existing <- objs[vapply(objs, exists, logical(1), inherits = TRUE)]
    
  ALL_DATA  <- if (length(existing) > 0) {
      do.call(rbind, lapply(existing, get))
    } else { stop("`There are no fishery data available for this date range/area and species", call. = FALSE)}
  ...
  if(age_length=="AGE"){
    ...
    if(nrow(srv_age)==0){ stop("There are no survey age data available to create the age length key for the date range/area")}

    ## add NMFS area to the survey data
   ...
   if(nrow(fish_raw)==0){ stop("There are no fishery age data available to create the age length key for the date range/area")}
    ...
    ALL_DATA <- predict_age_from_lf(
      ...
      seed = 1L)
  ## from here on LENGTH = AGE for calculations
    ALL_DATA$LENGTH <- ALL_DATA$AGE_HAT
  }
  ...
  CATCHT <- sql_run(con_akfin, catch) |>
  data.table::as.data.table()
  data.table::setnames(CATCHT, toupper(names(CATCHT)))
  
  CATCHT <- CATCHT[YEAR >= start_year & YEAR <= ly]
